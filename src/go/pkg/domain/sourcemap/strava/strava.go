// Package strava maps Strava API payloads (detailed activity + streams) onto the
// StandardizedActivity model. Shared by the webhook source and the historical-import
// source plugin so both produce identical activities.
package strava
...
// MapToStandardizedActivity builds a StandardizedActivity from a DetailedActivity JSON body
// and (optionally) its key_by_type StreamSet. With streams, per-sample records go into a
// single telemetry lap; without, Strava's summary laps are used.
...
// BuildRecordsFromStreams converts a key_by_type StreamSet into per-sample records.
...
// MapActivityType maps a Strava sport/activity type onto the FitGlue ActivityType enum.
...
var StreamKeys = []stravaapi.GetActivityStreamsParamsKeys{
